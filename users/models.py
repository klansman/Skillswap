...
class SwapRequest (models.Model):
    ...
    reciever_skill = models.ForeignKey(Skill, on_delete=models.CASCADE, related_name='recieving_skill')
    status =  models.CharField(max_length=20, choices=[('pending', 'Pending'), ('accepted', 'Accepted'), ('rejected', 'Rejected')], default='pending')
    created_at = models.DateField(auto_now_add=True)

    def __str__(self):
        return f"{self.sender} wants to swap {self.sender_skill} with {self.reciever_skill} of {self.reciever}"